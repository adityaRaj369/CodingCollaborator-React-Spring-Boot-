...
//             photoURL: location.state?.photoURL || "",
//             email: userEmail,
//             socketId: socketRef.current.id, // STOMP client doesn't have id, handled by backend
//           };
//           socketRef.current.emit(ACTIONS.JOIN, joinData);
//         });
...
//   const fetchSavedCodes = async () => {
//     if (!userEmail) return toast.error("Please log in to fetch codes");
//     try {
//       const response = await axios.get(`http://localhost:5000/api/saved-codes/${userEmail}`);
//       setSavedCodes(response.data);
//     } catch (error) {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        socketRef.current.on("connect_error", handleErrors, roomId);
        socketRef.current.on("connect_failed", handleErrors, roomId);
